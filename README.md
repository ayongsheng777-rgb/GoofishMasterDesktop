...
- **GitHub Releases**：[v1.1.5 安装包（约 581MB）](https://github.com/ayongsheng777-rgb/GoofishMasterDesktop/releases/tag/v1.1.5)

> v1.1.5 新增：瑕疵定义词**智能双向展开**（如「摔坏的手机」自动展开多词采集）、**发布时间筛选**（「最近3天」只采新发布）、**瑕疵词库三层增量进化**（自动记忆新表述）。历史修复详见 [RELEASE_NOTES.md](RELEASE_NOTES.md)。
...
- **瑕疵词智能展开**：搜索/监控「坏的 / 摔坏 / 屏幕破 / 变形」等瑕疵定语，自动按「瑕疵词 × 设备词」交叉展开为多个实际采集词（如「摔坏的手机」→ 摔坏的iphone / 屏幕破的手机 / 摔坏的苹果手机），监控任务零改动自动生效
- **发布时间筛选**：指令加「最近3天 / 7天内发布」，只保留最近发布的商品（UI 筛选尽力 + 采集后二次过滤兜底）
- **词库自进化**：内置口语/事件型瑕疵表述（车压了 / 可乐倒了 / 键盘不能用 / 跑气…），未命中时调 AI 判定，并在每次搜索后自动挖掘新表述入库（像 AI 一样记忆）
...
在飞书会话中直接发消息即可，支持自然语言，无需记命令：

- `找 摔坏的手机 最近3天` — 搜索瑕疵机，只采最近 3 天发布
- `找 坏的 笔记本电脑` — 裸瑕疵词自动展开为「坏的手机 / 坏的iphone / 坏的笔记本 / 坏的平板」逐词采集
- `监控 iphone 7天内发布` — 持续监控，只推新发布商品
- `找 RTX4090 个人卖家 价格8000以下` — 常规条件组合
- `帮助搜索` / `帮助监控` — 查看专题使用说明

> 瑕疵定语（坏的 / 摔坏 / 屏幕破 / 变形 / 进水 / 漏气…）会自动交叉设备词展开；时间条件（最近N天）贯穿搜索与监控。发送「帮助」查看完整指令。
