...
# V2.0: AI calls go through the ai-router service (unified routing/fallback).
# Set FORCE_DIRECT_AI=true to bypass the router and use OPENAI_* directly.
AI_ROUTER_URL = os.getenv("AI_ROUTER_URL", "").rstrip("/")
FORCE_DIRECT_AI = os.getenv("FORCE_DIRECT_AI", "false").lower() == "true"

if AI_ROUTER_URL and not FORCE_DIRECT_AI:
    API_KEY = os.getenv("OPENAI_API_KEY") or "via-ai-router"
    BASE_URL = f"{AI_ROUTER_URL}/v1"
    # "auto" lets the router pick the best configured model (fallback chain)
    MODEL_NAME = os.getenv("OPENAI_MODEL_NAME") or "auto"
    print(f"[AI配置] 已通过 ai-router 统一路由: {BASE_URL} (model={MODEL_NAME})")
else:
