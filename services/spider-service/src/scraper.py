...
async def scrape_user_profile(context, user_id: str) -> dict:
    ...
    try:
        ...
        # 无新数据 4s 即停（旧逻辑固定硬等 8s，是单件 15s 的主要来源）；
        # nextPage=False 仍即时终止，20 次滚动兜底防死循环
        for _ in range(20):
            if stop_item_scrolling.is_set():
                break
            prev_count = len(all_items)
            await page.evaluate("window.scrollTo(0, document.body.scrollHeight)")
            waited = 0.0
            while waited < 4.0:
                if stop_item_scrolling.is_set() or len(all_items) > prev_count:
                    break
        ...
        if await rating_tab_locator.count() > 0:
            ...
            # 评价列表同样改"无新数据 4s 即停"
            for _ in range(20):
                if stop_rating_scrolling.is_set():
                    break
                prev_count = len(all_ratings)
                await page.evaluate("window.scrollTo(0, document.body.scrollHeight)")
                waited = 0.0
                while waited < 4.0:
                    if stop_rating_scrolling.is_set() or len(all_ratings) > prev_count:
                        break
                    await asyncio.sleep(0.25)
                    waited += 0.25
                if stop_rating_scrolling.is_set():
                    break
                if len(all_ratings) == prev_count:
                    print("      [无新数据] 评价列表加载完毕。")
                    break
...
# 最近一次采集的业务级失败码（login_expired / risk_control），API 层读后透传给
# 调用方。引擎按"返回处理数"设计，业务错误原本只进日志——调用方无法区分
# 「真的 0 结果」和「登录过期 0 结果」（2026-08-03 实锤：用户被误导去换关键词，
# 真实解法应是重新扫码登录）。
LAST_SCRAPE_ERROR = ""
...
async def scrape_xianyu(task_config: dict, debug_limit: int = 0):
                    ...
                    for i, item_data in enumerate(basic_items, 1):
                        ...
                        try:
                            ...
                            if detail_response.ok:
                                ...
                                # 3. 提取商品视频（如有）：视频直链 + 封面图。
                                # 2026-08-03 诊断日志实录：真实字段是
                                # itemDO.videoPlayInfo = {major, ld320pUrl,
                                # sd480pUrl, videoId, widthSize, heightSize, ...}；
                                # 保留多候选兜底，诊断日志继续观察其他形态。
                                _vkeys = [k for k in item_do.keys()
                                          if "video" in str(k).lower()]
                                if _vkeys:
                                    print(f"[详情字段] 视频相关键: {_vkeys} -> "
                                          f"{json.dumps({k: item_do[k] for k in _vkeys}, ensure_ascii=False, default=str)[:600]}")
                                _video_info = None
                                for _vk in ("videoPlayInfo", "videoInfo",
                                            "videoDO", "video", "videoInfoDO"):
                                    _cand = item_do.get(_vk)
                                ...
                                _video_cover = ""
                                if _video_info:
                                    for _k in ("sd480pUrl", "hdUrl", "url",
                                               "videoUrl", "playUrl",
                                               "videoPlayUrl", "ld320pUrl"):
                                        _v = _video_info.get(_k)
                                        if isinstance(_v, str) and \
                                                _v.startswith("http"):
                                            _video_url = _v
                                            break
                                    for _k in ("coverImage", "coverUrl",
                                               "poster", "posterImage",
                                               "cover", "picUrl"):
                                        _v = _video_info.get(_k)
                                        if isinstance(_v, str) and \
                                            ...
                                            break
                                if not _video_url:
                                    _v = item_do.get("videoUrl")
                                    if isinstance(_v, str) and \
                                            _v.startswith("http"):
                                        _video_url = _v
                                if _video_url:
                                    item_data["商品视频链接"] = _video_url
                                    print(f"[视频] 商品含视频: {_video_url[:80]}")
                                if _video_cover:
                                    # 视频封面并入图片列表，视觉链可据此判断视频内容
                                    _imgs = item_data.setdefault("商品图片列表", [])
                                    if _video_cover not in _imgs:
                                        _imgs.append(_video_cover)
    ...
    for attempt in range(1, attempt_limit + 1):
        ...
        try:
            ...
            last_error = str(e)
            LAST_SCRAPE_ERROR = "login_expired"
            print(f"检测到登录失效/重定向: {e}")
            break
        except RiskControlError as e:
            last_error = str(e)
            LAST_SCRAPE_ERROR = "risk_control"
